/**
 * NEXUS Crypto Service
 * BIP-39 mnemonic, Ed25519 DID identity, AES-256-GCM encryption
 * Uses Web Crypto API + @noble/curves (zero other dependencies)
 */
import { ed25519 } from 'https://esm.sh/@noble/curves@1/ed25519';
...
/**
 * Generate a 12-word BIP-39 mnemonic from 128 bits of entropy.
...
export async function generateMnemonic() {
  ...
  // Build bit string: 128 entropy bits + 4 checksum bits = 132 bits
  const bits = new Uint8Array(132);
  for (let i = 0; i < 16; i++) {
    for (let j = 0; j < 8; j++) {
...
/**
 * Validate a BIP-39 mnemonic (12 words, valid checksum).
...
/**
 * BIP-39: Derive 512-bit seed from mnemonic using PBKDF2.
 * @param {string} mnemonic
...
async function mnemonicToSeed(mnemonic) {
  ...
  const seed = await crypto.subtle.deriveBits(
    {
      name: 'PBKDF2',
      salt: enc.encode('mnemonic'),  // BIP-39 standard (empty passphrase)
      iterations: 2048,
      hash: 'SHA-512',
    },
    keyMaterial,
...
/**
 * HKDF: Derive bytes from seed with a context label.
 * @param {Uint8Array} seed
 * @param {string} info - Context label (e.g. "nexus-identity")
 * @param {number} length - Output bytes
 * @returns {Promise<Uint8Array>}
 */
async function hkdfDerive(seed, info, length = 32) {
  const keyMaterial = await crypto.subtle.importKey(
...
/**
 * Derive a complete identity from a mnemonic:
 * - Ed25519 key pair (for DID + future signing)
 * - AES-256-GCM storage key (for IndexedDB encryption)
 *
 * All keys are deterministic — the same mnemonic always produces
 * the same identity, enabling recovery.
 *
 * @param {string} mnemonic - 12-word BIP-39 mnemonic
 * @returns {Promise<{did: string, publicKey: Uint8Array, privateKey: Uint8Array, storageKey: CryptoKey}>}
 */
export async function deriveIdentity(mnemonic) {
  const seed = await mnemonicToSeed(mnemonic);

  // Ed25519 identity key pair (deterministic from seed)
  const privateKey = await hkdfDerive(seed, 'nexus-identity', 32);
  const publicKey = ed25519.getPublicKey(privateKey);

  // DID from public key
  const did = publicKeyToDid(publicKey);

  // AES-256-GCM storage key (non-extractable)
  const storageKeyBytes = await hkdfDerive(seed, 'nexus-storage', 32);
  ...
  return { did, publicKey, privateKey, storageKey };
}

/**
 * Encode an Ed25519 public key as a did:key URI.
 * Format: did:key:z{base58btc(multicodec_ed25519_pub + raw_key)}
 *
 * @param {Uint8Array} publicKey - 32-byte Ed25519 public key
 * @returns {string} did:key URI
 */
 ...
 * @returns {Promise<Uint8Array>} Raw decrypted bytes
 */
 ...
 */
export async function decryptToString(key, data) {
